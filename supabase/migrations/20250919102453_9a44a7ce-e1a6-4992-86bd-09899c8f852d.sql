...
BEGIN
  ...
  -- Check if called by service role (auth.email() returns null for service role)
  is_service_role := (current_user_email IS NULL);
  
  -- If not called by service role, apply security checks
