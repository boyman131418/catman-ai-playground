...
function signJob(job){
  return crypto.createHmac('sha256',token()).update(String(job)).digest('base64url');
}
...
async function flatJson(path,opts={}){
  const r=await fetch(`https://api.flat.io/v2${path}`,{
    ...opts,
    headers:flatHeaders({'Content-Type':'application/json',...(opts.headers||{})})
  });
...
export default async function handler(req,res){
  ...
  try{
    if(req.method==='GET' && action==='capabilities'){
      const {r,data}=await flatJson('/omr/capabilities',{method:'GET'});
      if(!r.ok) return flatError(res,r,data,'Unable to read OMR capabilities');
      return res.status(200).json({ok:true,configured:true,provider:'professional-omr',capabilities:data});
    }
    ...
    if(req.method==='POST' && action==='start'){
      ...
      if(file.length>4_000_000) return res.status(413).json({ok:false,error:'File is too large for the web upload gateway. Please use a clearer/compressed image under about 3 MB.',code:'FILE_TOO_LARGE'});
      ...
      const payload={
        ...
        interactiveSteps:[],
        locales:[locale],
        idempotencyKey:`pianoforge-${key}`,
        files:[{file,filename,mimeType}],
        autoStart:true
      };
      const {r,data}=await flatJson('/omr/jobs',{method:'POST',body:JSON.stringify(payload)});
      if(!r.ok) return flatError(res,r,data,'Unable to start professional OMR');
    ...
    if(req.method==='GET' && action==='status'){
      ...
      return res.status(200).json({
        ...
        progress:data.progress??null,
        estimatedCredits:data.estimatedCredits??null,
        importedMetadata:data.importedMetadata??null,
    ...
    if(req.method==='GET' && action==='result'){
      ...
      if(!r.ok){
        const data=await r.json().catch(()=>({}));
        return flatError(res,r,data,'Unable to download MusicXML');
      }
